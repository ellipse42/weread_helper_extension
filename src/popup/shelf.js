...
chrome.storage.local.get(['userShelf'], function(result) {
  ...
  if (shelf && shelf.books && shelf.books.length > 0) {
    ...
    for (var i=0; i<books.length; i++) {
      _ui += `
      <div>
        <a class="webook_book_item" href="https://weread.qq.com/web/reader/${make_me_happy(books[i].bookId)}" target="_blank">${books[i].title}</a>
      </div>
      `
    }
    $('#webook_user_shelf').prepend($(`${_ui}`))
  } else {
    let _ui = $(`
